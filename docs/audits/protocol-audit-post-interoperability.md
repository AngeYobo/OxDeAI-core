...
| Policy binding (`policy_id`) | `PARTIAL` | The verifier compares `policy_id` against a caller-supplied `expectedPolicyId` (conformance vector `auth-policy-id-mismatch`). **Historical note:** this row previously read `DONE`. That overstated the boundary: at the affected enforcement paths the expectation was derived from the artifact under verification (`expectedPolicyId: authorization.policy_id`), so the comparison could not fail. Corrected in #301 — the direct guard now derives the expectation from `engine.computePolicyId()` (trusted engine configuration), and the gateway and delegation-root paths require deployer-configured issuer-policy authority instead. |
| Issuer binding | `PARTIAL` | `verifyAuthorization` checks `AUTH_ISSUER_MISMATCH` against a caller-supplied `expectedIssuer` (conformance vector `authorization-sig-004`), and key resolution is issuer-scoped: `findKeyInKeySets()` selects a key set by `keyset.issuer === authorization.issuer`, so a claimed issuer must have a deployer-configured key set. **Historical note:** this row previously read `DONE`. A valid signature proves only that a trusted key for the *claimed* issuer signed the artifact — it never proved that this issuer is authorized to issue for the claimed `policy_id`. Corrected in #301. |
| Issuer-policy authority | `DONE` | Added by #301. Externally supplied authorizations (PEP gateway, delegation `parentAuth`) must match a deployer-configured `(issuer, policyId)` pair via `isTrustedAuthorizationAuthority`; a mismatch fails closed with `AUTH_ISSUER_POLICY_NOT_AUTHORIZED`. Authority is a complete pair, never two independent allow-lists, so `A/P1 + B/P2` does not authorize `A/P2` or `B/P1`. Missing configuration fails at gateway construction; an explicitly empty list is valid and authorizes nothing. |
